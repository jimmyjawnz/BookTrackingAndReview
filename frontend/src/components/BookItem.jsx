...
const BookItem = ({ title, author, description, imageUrl, rating, reviews }) => {
  const truncatedDescription = description.length > 100 ? description.slice(0, 100) + "..." : description;

  return (
    <article className="flex flex-wrap md:flex-nowrap shadow-lg mx-auto max-w-[600px] max-h-[220px] group cursor-pointer mb-3">
      <img
      ...
      <div className="flex flex-col justify-between p-3 w-full">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1 leading-tight">{truncatedDescription}</p>
        </div>
        <div className="bg-blue-50 p-2 rounded-md">
          <div className="flex justify-between items-center">
            <div>
              <div className="text-xs text-gray-700">
                <span className="font-bold text-gray-900">Author:</span> {author}
              </div>
              <div className="flex items-center mt-1">
                ...
                <span className="text-gray-600 ml-2 text-xs">{reviews} reviews</span>
              </div>
            </div>
